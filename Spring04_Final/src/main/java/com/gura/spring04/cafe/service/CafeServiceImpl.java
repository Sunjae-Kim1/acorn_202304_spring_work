...
public class CafeServiceImpl implements CafeService{
	...
	// 페이징 처리 , 검색어 기능을 고려한 비즈니스 로직을 처리하는 메소드
	@Override
	...
	public void saveContent(CafeDto dto) {
		// title , content , writer 정보가 들어있는 CafeDto 를 dao 에 전달해서 DB 에 저장한다.
		cafeDao.insert(dto);
	}
	...
	// 글 수정에 필요한 데이터 값을 HttpServletRequest 에 담아주는 메소드 
	@Override
	...
	public void saveComment(HttpServletRequest request) {
		// 폼 전송되는 파라미터 추출
		int ref_group =  Integer.parseInt(request.getParameter("ref_group")); 	// 댓글의 글 번호
		...
		if(comment_group == null) {
			...
			// 전송된 comment_group 번호를 숫자로 바꾸고 Dto 에 넣어준다.
	...
	public void deleteComment(HttpServletRequest request) {
		...
		if(!dto.getWriter().equals(id)) {
			throw new NotDeleteException("남의 댓글을 지우면 않됩니다.");
		}
	...
	@Override
	public void moreCommnetList(HttpServletRequest request) {
		// 로그인된 아이디
		String id = (String)request.getSession().getAttribute("id");
		// ajax 요청 파리미터로 넘어오는 댓글의 페이지 번호를 읽어낸다.
		int pageNum = Integer.parseInt(request.getParameter("pageNum"));
		// ajax 요청 파리미터로 넘어오는 원 글의 번호를 읽어낸다.
		int num = Integer.parseInt(request.getParameter("num"));
