...
<head>
<meta charset="UTF-8">
<title>/gallery/upload_form3.jsp</title>
<style>
	#dropZone{
		width: 400px;
		height: 400px;
...
<body>
	...
	<script>
		...
		document.querySelector("#submitBtn").addEventListener("click" , () => {
			...
			// 만일 caption 을 3 글자 미만으로 입력하거나 선택한 파일이 없으면 
			...
			// data.append("파라미터 명" , 데이터);
			// caption 이라는 파라미터 명으로 input 요소에 입력한 문자열을 담는다.
			data.append("caption" , caption);
			// input 요소에 선택한 파일 데이터를 얻어오기
			const file = files[0];
			...
			*/
			...
			.then(data => {
				console.log(data);
				if(data.isSuccess){
					alert(file.name + " 이미지를 성공적으로 업로드를 했습니다.");
					// UI 를 초기 상태로 되돌리기
