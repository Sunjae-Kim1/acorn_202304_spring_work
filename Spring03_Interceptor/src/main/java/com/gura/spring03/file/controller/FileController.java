...
public class FileController {
	...
	public Map<String , Object> imageUpload(MultipartFile image , HttpServletRequest request) {
		//1. 원본 파일명
				...
				// "imagePath" 라는 키 값으로 업로드된 이미지를 요청할 수 있는 경로를 담아서
				map.put("imagePath", "/resources/upload/" + saveFileName);
				// 리턴하면 {"imagePath":"xxx"} 형식의 JSON 문자열이 응답한다.
				return map;
			}
	...
	public String upload(FileDto dto , HttpServletRequest request) { // FileDto 의 필드에 MultipartFile 객체의 참조값이 들어있다.
		// FileDto 객체에 들어있는 MultipartFile 객체를 이용해서 파일 업로드 관련 처리한다.
	...
	/*
	 * [ 파일 업로드 요청 처리 ]
	 * title 이라는 파라미터 명으로 제목 ( 파일의 실명 )
	 * myFile 이라는 파라미터 명으로 첨부 파일이 전송된다.
	 */
	...
	public String upload(String title , MultipartFile myFile , HttpServletRequest request) {
		/*
		 * 입력한 제목은 title 에 들어오고 , 업로드된 파일에 대한 정보는 myFile 객체에 들어있다.
		 * 따라서 myFile 객체의 메소드를 활용해서 업로드 처리에 필요한 정보를 얻어내고 어떤 동작을 수행하면 된다.
		 * 
		 * 참고로 일반적인 웹 서버 Application 은 클라이언트가 파일 업로드를 해오면 운영체계의 임시폴더에
		 * 이상한 ( 전 ) 파일 명으로 일단 저장을 해놓는다.
		 * 
		 * 일정시간 이후에 임시 폴더에 저장되었던 파일은 운영체계가 자동으로 지우거나 비운다.
		 * 
