...
public class MemberController {
	 ...
	 * 경로만 맞으면 모두 처리한다.( Get , Post , Put , Delete )
	 * 
	 * method 속성의 값을 명시하면 경로가 맞더라도 요청 방식이 다르면 처리하지 않는다.
	 * 일반적을 Post 인 경우에는 요청 방식을 명시하는 것이 좋다.
	 */
	...
	public ModelAndView updateform(ModelAndView mView ,int num) {
		...
		/*
		 * 수행할 회원의 정보를 ModelAndView 객체의 addObject( key , value ) 메소드를 이용해서 담는다.
		 * ModelAndView 객체에 담은 값은 결국 HttpServletRequest 객체에 담긴다.( Request Scope 에 담긴다. )
		 */
		mView.addObject("dto" , dto);
		
		// View Page 의 위치로 ModelAndView 객체에 담아서 리턴
	...
	/*
	 * @RequestParam(value = "파라미터 명"  , defaultValue = "기본 값")
	 * 기본 값은 없어도 되고 파라미터 명과 매개변수 명이 일치한다면 생략 가능
	*/
	// 회원 삭제 요청 처리
