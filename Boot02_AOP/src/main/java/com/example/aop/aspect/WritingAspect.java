 ...
 *		메소드의 첫번째 인자는 Integer type, 두번째 인자는 모든 type 이 되는
 *		메소드가 point cut (AOP 가 적용되는 위치)
 */
...
public class WritingAspect {
	 ...
	 * 메소드의 매개변수 	=> 없음
	 * 
	 * Aspect 가 적용되는 위치를 "Point Cut" 이라고 한다.
	 */
	
	@Before("execution(void write*())")
	public void prepare() {
		System.out.println("파란색 Pen 을 준비합니다."); // 횡단 관심사 Cross Concern
	}
	
	@After("execution(void write*())")
	public void end() {
		System.out.println("Pen 을 닫고 마무리 합니다."); // 횡단 관심사 Cross Concern
	}
}