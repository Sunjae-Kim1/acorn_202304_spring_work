...
public class CafeController {
	...
	public Map<String , Object> commentUpdate(CafeCommentDto dto){
		...
		// {"isSuccess" : true} 형식의 JSON 문자열이 응답하도록 한다.
		return map;
	}
	...
	public String commentList(HttpServletRequest request) {
		...
		service.moreCommnetList(request);
	...
	public String commentInsert(HttpServletRequest request , int ref_group) {
		...
		//ref_group 은  원글의 글 번호이기 때문에 자세히 보기로 다시 리다이렉트 이동한다.
	...
	public String delete(int num, HttpServletRequest request) {
		// 서비스에 삭제할 글 번호와 HttpServletRequest 객체를 전달해서 해당 글을 삭제시키고
